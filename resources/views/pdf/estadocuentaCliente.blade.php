    ...
    <section>
        ...
        <table width="100%" border="1" class="table th td">
            ...
            <tr>
                ...
                @if($rec->moneda=='MXN')
                    ...
                    <td align="right">{{$rec->monto}}</td><!--Importe en MXN-->
                    <td align="right">{{number_format($rec->monto/$rec->intercam,2)}}</td><!--Importe en USD-->
                    
                    @else
                    <td align="right">{{number_format($rec->monto*$rec->intercam,2)}}</td><!--Importe en MXN-->
                    <td align="right">{{$rec->monto}}</td><!--Importe en USD-->
        ...
        <table width="36%" align="right">
            <tr>
                <td> </td>
                <td> Total Pagado </td>
                <td>{{number_format($montomxn,2)}}</td><!--MXN-->
                <td>{{number_format($montousd,2)}}</td><!--USD-->
            </tr>
            <tr>
                <td> </td>
                <td>Dif. a Pagar</td>
            ...
            <tr>
                <td> </td>
                <td class="t2" colspan="4"> Elaboró: {{ Auth::user()->cid_empleado}} - {{Auth::user()->cnombre }} {{Auth::user()->capellidop }} {{Auth::user()->capellidom }} {{$hoy}} </td>
            </tr>
        </table>
